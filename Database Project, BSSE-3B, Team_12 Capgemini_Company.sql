...
--						 4)	Show the record of employees by decreasing age

select E.EmployeeID, E.[Name], E.[Address], year(getdate())-year(E.birth_date) as Age ,E.Salary, E.Birth_Date, E.Phone_No from Employee E 
order by year(getdate())-year(E.birth_date) desc


--						 5)	Show the record of oldest (in age) employee

select top 1 E.EmployeeID, E.[Name], E.[Address], year(getdate())-year(E.birth_date) as Age ,E.Salary, E.Birth_Date, E.Phone_No from Employee E 
order by year(getdate())-year(E.birth_date) desc


--						 6)	Generate Username for all employees & Show EmployeeID, Usernames, Name, Salary, Phone_No for all the employees

select E.EmployeeID, 
SUBSTRING(E.EmployeeID,1,3) + SUBSTRING(E.EmployeeID,7,1) + '.' + 
SUBSTRING(upper(E.[Name]),1,1) + SUBSTRING(lower(E.[Name]),2,4) + 
cast(day(E.Birth_Date) as varchar(2)) as [Generated UserName], 
E.[Name], E.[Address], E.Birth_Date, E.Salary, E.Phone_No from employee E


--						 7)	Show How Many Employees Are Working on Projects in ascending order

select P.ProjectID, P.Project_Name as [Project Name], P.Starting_Date as [Project Starting Date], count(PE.EmployeeID) as [Number of Employees] 
from Project P
left join Pro_Emp PE
on PE.ProjectID = P.ProjectID
group by P.ProjectID, P.Project_Name, P.Starting_Date
order by count(PE.EmployeeID) asc


--						 8)	Show ClientID, Client Name, Project Name for entered "Client ID" (Shows data of all clients if entered "null")

/*

create function Client_Projects(@CID nvarchar(8))
returns table
as
return (select C.ClientID, C.[Name], P.Project_Name, CO.Contract_date from Client C
		join [Contract] Co
		on C.ClientID = CO.ClientID

		join Project P
		on P.ProjectID = CO.ProjectID
	
		where C.ClientID = isnull(@CID, C.ClientID))

*/

select * from dbo.Client_Projects('C20-001')
